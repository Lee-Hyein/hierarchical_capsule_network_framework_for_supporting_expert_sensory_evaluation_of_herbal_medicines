...
DEFAULT_CLASS_COUNTS = {'master': 4, 'sub': 21, 'article': 45}


def _resolve_class_counts(class_counts: Optional[Dict[str, int]]) -> Dict[str, int]:
    resolved = DEFAULT_CLASS_COUNTS.copy()
    if class_counts is None:
        return resolved
    for key in resolved:
        if key in class_counts and class_counts[key] is not None:
            resolved[key] = int(class_counts[key])
    return resolved
...
class ConditionTrain:
    ...
    def __init__(
        ...
        counts = _resolve_class_counts(class_counts)
        self.master_classes = counts['master']
        self.sub_classes = counts['sub']
        self.art_classes = counts['article']
        
        '''Three inputs to model for training, image, labels ,labels for teacher forcing'''
        input_image = Input(shape=input_shape,name="InputImg")

        #4 element vector for the masterCategory types
        input_master = Input(shape=(self.master_classes,), name="InputMaster")

        #21 element vector for the subCategory types
        input_sub = Input(shape=(self.sub_classes,), name="InputSub")
        ...
        metrics = {
            "master_output": "categorical_accuracy",
            "sub_output": "categorical_accuracy",
            "article_output": "categorical_accuracy"
        }
        model.compile(
